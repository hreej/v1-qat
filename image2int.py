...
QAT_MODEL_PATH = "quant_results/float2int/qat_checkpoints/litenet_int8_qat.pth"
# 2. 你的测试图片路径
IMAGE_PATH = "constract_high_zoom_22.jpg"  # <--- 修改为你的图片文件名
# 3. 输出头文件名称
OUTPUT_HEADER = "image/input_image.h"
OUTPUT_PACKED_HEADER = "image/input_image_packed.h"
OUTPUT_PACKED_HWC_HEADER = "image/input_image_packed_HWC.h"
OUTPUT_BIN = "image/image.bin"
OUTPUT_HWC_BIN = "image/image_HWC.bin"
# 4. 类别数
NUM_CLASSES = 12
# ========================================
...
def save_packed_hwc_to_header(img_np, filename):
    """将 int8 数据转为 HWC 顺序后打包为 int32 并保存为 C++ 头文件"""
    # img_np shape is (C, H, W) -> (3, 128, 128)
    # Transpose to (H, W, C) -> (128, 128, 3)
    img_hwc = np.transpose(img_np, (1, 2, 0))
    
    # 展平数组
    flat_data = img_hwc.flatten()
    ...
    print(f"[-] 生成 HWC 打包头文件: {filename}")
    with open(filename, "w") as f:
        f.write("// Auto-generated packed data header (HWC format) from image2int8.py\n")
        f.write(f"// Original Image: {IMAGE_PATH}\n")
        f.write("#ifndef INPUT_IMAGE_PACKED_HWC_H\n")
        f.write("#define INPUT_IMAGE_PACKED_HWC_H\n\n")
        f.write("#include <ap_int.h>\n\n")
        f.write(f"// Original elements: {len(flat_data)}\n")
        f.write(f"// Packed elements: {len(packed_data)}\n")
        f.write(f"static ap_int<32> input_packed_hwc_arr[{len(packed_data)}] = {{\n")
...
def save_packed_hwc_to_bin(img_np, filename):
    """将 int8 数据转为 HWC 顺序后打包为 int32 并保存为二进制文件"""
    # img_np shape is (C, H, W) -> (3, 128, 128)
    # Transpose to (H, W, C) -> (128, 128, 3)
    img_hwc = np.transpose(img_np, (1, 2, 0))
    ...
    print(f"[-] 生成 HWC 二进制文件: {filename}")
    with open(filename, 'wb') as f_out:
        for val in packed_data:
            f_out.write(struct.pack('<I', val))
    print(f"    HWC 二进制文件生成完成! 大小: {len(packed_data) * 4} 字节")
...
def main():
    ...
    if img_int8 is not None:
        ...
        save_packed_hwc_to_header(img_int8, OUTPUT_PACKED_HWC_HEADER)
        save_packed_to_bin(img_int8, OUTPUT_BIN)
        save_packed_hwc_to_bin(img_int8, OUTPUT_HWC_BIN)
